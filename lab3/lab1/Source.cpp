...
void Simulation()
{
	...
	bool CameraA = GetAsyncKeyState(0x41);
	bool CameraS = GetAsyncKeyState(0x53);
	bool CameraD = GetAsyncKeyState(0x44);
	bool CameraQ = GetAsyncKeyState(0x51);
	bool CameraE = GetAsyncKeyState(0x45);

	camera.setKey(CameraLeft, CameraRight,
		CameraUp, CameraDown,
		CameraForward, CameraBackward, 
		CameraW, CameraA, CameraS, CameraD, CameraQ, CameraE);
